...
export default function TestPage() {
  ...
  useEffect(() => {
    // Add global class to body to hide the old layout elements
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes starTwinkle { 0%, 100% { opacity: 0.25; transform: scale(0.85); } 50% { opacity: 0.95; transform: scale(1.2); } }
.star-twinkle { animation: starTwinkle infinite ease-in-out; will-change: transform, opacity; }

/* Micro-interaction & Transition System */
:root {
  --ease-spring: cubic-bezier(0.16, 1, 0.3, 1);
}

/* Continuous ambient breathing behind APS title */
@keyframes ambientBreathe {
  0%, 100% { opacity: 0.45; transform: scale(1.15); filter: blur(36px); }
  50% { opacity: 0.85; transform: scale(1.35); filter: blur(48px); }
}
.tube-ambient-breathe {
  animation: ambientBreathe 6s ease-in-out infinite;
}

/* Continuous logo subtle orbital spin and hover glow */
@keyframes logoSlowRotate {
  0% { transform: rotate(0deg); }
  100% { transform: rotate(360deg); }
}
.logo-orbit-ring {
  animation: logoSlowRotate 24s linear infinite;
...
/* Metric node card interactive hover lift & border sheen */
.interactive-metric-node {
  transition: transform 0.45s var(--ease-spring), border-color 0.4s ease, box-shadow 0.45s var(--ease-spring);
  will-change: transform, box-shadow;
}
.interactive-metric-node:hover {
  transform: translateY(-5px) scale(1.03);
}
.interactive-metric-node:hover .glass-pill {
  border-color: rgba(45, 212, 191, 0.45);
  box-shadow: 0 12px 35px -8px rgba(0, 0, 0, 0.8), 0 0 25px rgba(45, 212, 191, 0.28), inset 0 1px 1px rgba(255, 255, 255, 0.15);
}

/* Feature card enhanced hover physics & glow */
...
.interactive-feature-card:hover {
  ...
  box-shadow: 0 28px 65px -10px rgba(0, 0, 0, 0.8), 0 0 40px rgba(45, 212, 191, 0.14), inset 0 1px 2px rgba(255, 255, 255, 0.12);
}
.interactive-feature-card:hover .feature-icon-badge {
  transform: rotate(-6deg) scale(1.08);
  border-color: rgba(45, 212, 191, 0.7);
  box-shadow: 0 0 25px rgba(45, 212, 191, 0.5);
}
.interactive-feature-card:hover .feature-category-pill {
  border-color: rgba(45, 212, 191, 0.65);
  box-shadow: 0 0 16px rgba(45, 212, 191, 0.35);
  color: #a7f3d0;
}
.interactive-feature-card:hover .explore-btn {
  transform: scale(1.04);
  box-shadow: 0 0 28px rgba(45, 212, 191, 0.75);
}
.interactive-feature-card:hover .explore-arrow {
  transform: translateX(4px);
}

/* Indicator progress breathing & pulsing */
@keyframes barPulseGlow {
  0%, 100% { box-shadow: 0 0 8px rgba(52, 211, 153, 0.7); opacity: 0.9; }
  50% { box-shadow: 0 0 18px rgba(45, 212, 191, 1); opacity: 1; }
}
.progress-live-bar {
  animation: barPulseGlow 2.4s ease-in-out infinite;
}
...
@media (prefers-reduced-motion: reduce) {
  ...
  .logo-orbit-ring,
  .tube-ambient-breathe,
  .circuit-pulse-stream-outward,
  .circuit-sparkle-dash,
  .animate-float-slow,
  ...
  .btn-kinetic:hover {
    ...
    .hero-glow-teal {
      background: radial-gradient(ellipse 70% 50% at 75% 30%, rgba(20, 184, 166, 0.18), transparent 70%);
    }

    .hero-glow-sage {
      background: radial-gradient(ellipse 60% 45% at 85% 45%, rgba(140, 166, 153, 0.15), transparent 60%);
    }

    .hero-glow-dark-core {
      background: radial-gradient(circle at 35% 50%, rgba(5, 7, 7, 0.95), rgba(5, 7, 7, 0.2) 70%);
    }

    /* Subtle Circuit & Star Canvas */
    .stardust-pattern {
    ...
    /* Glassmorphism Border & Backdrops */
    .glass-pill {
      background: rgba(18, 24, 23, 0.65);
    ...
    .glass-card {
      background: linear-gradient(135deg, rgba(255, 255, 255, 0.03) 0%, rgba(255, 255, 255, 0.01) 100%), rgba(10, 14, 13, 0.7);
      backdrop-filter: blur(16px);
      -webkit-backdrop-filter: blur(16px);
      border: 1px solid rgba(255, 255, 255, 0.07);
      box-shadow: 0 20px 50px rgba(0, 0, 0, 0.5), inset 0 1px 1px 0 rgba(255, 255, 255, 0.07);
    }

    .glass-card:hover {
      border-color: rgba(52, 211, 153, 0.25);
      box-shadow: 0 25px 60px rgba(0, 0, 0, 0.65), 0 0 30px rgba(20, 184, 166, 0.08);
    }

    /* Metallic / Chrome Text Gradient */
    .text-metallic {
      background: linear-gradient(180deg, #FFFFFF 20%, #A1A8A5 100%);
    ...
    /* Fluorescent tube ignite animation */
    .flicker-neon-title {
      animation: fluorescentFlicker 2.6s cubic-bezier(0.23, 1, 0.32, 1) forwards;
      will-change: opacity, filter;
    }

    .tube-ambient-halo {
      animation: haloFadeIn 2.6s ease-out forwards;
    }
    ...
    /* Continuous Circuit Electrical Pulse Animations (Outward from Center) */
    .circuit-track {
    ...
    .circuit-pulse-stream-outward {
      ...
      animation: circuitStreamFlowOutward 3.2s linear infinite;
      filter: drop-shadow(0 0 6px rgba(45, 212, 191, 0.95)) drop-shadow(0 0 12px rgba(20, 184, 166, 0.6));
    }
    ...
    @keyframes circuitStreamFlowOutward {
      0% {
    ...
    .energy-beacon {
      animation: energyBeacon 2s cubic-bezier(0.4, 0, 0.6, 1) infinite;
      transform-origin: center;
    }

    /* Minimal Celestial Constellations */
    .constellation-group {
      opacity: 0.5;
      cursor: pointer;
    ...
    .constellation-line {
      ...
      fill: none;
      transition: stroke 0.4s cubic-bezier(0.16, 1, 0.3, 1), stroke-width 0.4s ease;
    }
    ...
    .btn-kinetic:active {
      ...
      <div className="bg-brand-pitch text-slate-200 overflow-x-hidden min-h-screen relative selection:bg-teal-500/30 selection:text-teal-200">
        <svg aria-hidden="true" className="inline-defs-container" style={{position: 'absolute', width: '0', height: '0', overflow: 'hidden'}}><defs><linearGradient id="brandLogoGrad" x1="0%" x2="100%" y1="0%" y2="100%"><stop offset="0%" stopColor="#2dd4bf" /><stop offset="50%" stopColor="#06b6d4" /><stop offset="100%" stopColor="#10b981" /></linearGradient><linearGradient id="brandOrbGrad" x1="100%" x2="0%" y1="0%" y2="100%"><stop offset="0%" stopColor="#34d399" /><stop offset="50%" stopColor="#14b8a6" /><stop offset="100%" stopColor="#06b6d4" /></linearGradient></defs></svg>
...
<div className="absolute inset-0 bg-gradient-to-t from-brand-pitch via-transparent to-brand-pitch/80"></div>
...
<a className="px-4 py-1.5 text-xs font-medium text-white bg-white/10 rounded-full transition-all duration-300 shadow-sm border border-white/15 hover:bg-white/20 hover:shadow-[0_0_15px_rgba(255,255,255,0.2)]" href="#hero">Home</a>
<a className="px-4 py-1.5 text-xs font-medium text-zinc-300 hover:text-white rounded-full hover:bg-white/[0.08] hover:shadow-[0_0_12px_rgba(45,212,191,0.15)] transition-all duration-300" href="/leaderboard">Leaderboard</a>
<a className="px-4 py-1.5 text-xs font-medium text-zinc-300 hover:text-white rounded-full hover:bg-white/[0.08] hover:shadow-[0_0_12px_rgba(45,212,191,0.15)] transition-all duration-300" href="/subject">Subjects</a>
<a className="px-4 py-1.5 text-xs font-medium text-zinc-300 hover:text-white rounded-full hover:bg-white/[0.08] hover:shadow-[0_0_12px_rgba(45,212,191,0.15)] transition-all duration-300" href="/search">Search</a>
<div className="w-px h-3.5 bg-white/15 mx-1"></div>
<a className="px-3.5 py-1.5 text-xs font-medium text-teal-300 hover:text-teal-100 rounded-full hover:bg-teal-500/10 hover:shadow-[0_0_16px_rgba(45,212,191,0.3)] transition-all duration-300 flex items-center gap-1" href="/admin">Over Power</a>
</nav>

<div className="flex items-center gap-3">
<a href="/upload" className="glass-pill hover:bg-white/[0.09] text-white text-xs font-medium py-2 px-4 rounded-full transition-all duration-300 flex items-center gap-2 border border-white/15 hover:border-teal-400/50 hover:shadow-[0_0_20px_rgba(20,184,166,0.3)] hover:scale-102 active:scale-98 group">
...
<stop offset="40%" stopColor="#2dd4bf" stopOpacity="0.32" />
<stop offset="100%" stopColor="#34d399" stopOpacity="0.1" />
</linearGradient>
<linearGradient id="trackGradRight" x1="0%" x2="100%" y1="0%" y2="0%">
<stop offset="0%" stopColor="#ffffff" stopOpacity="0.2" />
<stop offset="40%" stopColor="#34d399" stopOpacity="0.32" />
...
<path className="circuit-sparkle-dash" d="M 420 224 H 345 L 280 180 H 210" id="pathTopLeft" opacity="0.65" stroke="url(#trackGradLeft)" strokeWidth="1.2" />
<path className="circuit-pulse-stream-outward" d="M 420 224 H 345 L 280 180 H 210" filter="url(#neonFilter)" stroke="url(#glowPulseTealOut)" />



<path className="circuit-sparkle-dash" d="M 430 270 H 380 L 310 348 H 235" id="pathBottomLeft" opacity="0.65" stroke="url(#trackGradLeft)" strokeWidth="1.2" />
<path className="circuit-pulse-stream-outward" d="M 430 270 H 380 L 310 348 H 235" filter="url(#neonFilter)" stroke="url(#glowPulseTealOut)" style={{animationDelay: '-1.4s'}} />



<path className="circuit-sparkle-dash" d="M 775 224 H 840 L 910 168 H 985" id="pathTopRight" opacity="0.65" stroke="url(#trackGradRight)" strokeWidth="1.2" />
<path className="circuit-pulse-stream-outward" d="M 775 224 H 840 L 910 168 H 985" filter="url(#neonFilter)" stroke="url(#glowPulseCyanOut)" style={{animationDelay: '-0.7s'}} />



<path className="circuit-sparkle-dash" d="M 770 270 H 815 L 885 348 H 960" id="pathBottomRight" opacity="0.65" stroke="url(#trackGradRight)" strokeWidth="1.2" />
<path className="circuit-pulse-stream-outward" d="M 770 270 H 815 L 885 348 H 960" filter="url(#neonFilter)" stroke="url(#glowPulseCyanOut)" style={{animationDelay: '-2.1s'}} />

<line stroke="white" strokeOpacity="0.18" strokeWidth="1" x1="540" x2="540" y1="360" y2="480" />
<line stroke="teal" strokeOpacity="0.3" strokeWidth="1" x1="590" x2="590" y1="340" y2="530" />
<line stroke="white" strokeOpacity="0.15" strokeWidth="1" x1="640" x2="640" y1="370" y2="500" />
</svg>
...
<span className="text-sm font-mono font-bold text-white tracking-tight group-hover:text-emerald-300 transition-colors"><AnimatedNumber value={stats?.top_sgpa || 0} decimals={2} /></span>
<span className="text-[9px] font-mono text-emerald-400 bg-emerald-500/10 px-1 py-0.2 rounded border border-emerald-500/20 group-hover:bg-emerald-500/20 group-hover:border-emerald-400/40 transition-all">+0.42</span>
</div>
</div>
</div>
</div>
...
<span className="text-sm font-mono font-bold text-white tracking-tight group-hover:text-teal-200 transition-colors"><AnimatedNumber value={stats?.clean_records || 0} /></span>
...
<span className="text-xs font-mono text-zinc-400 group-hover:text-zinc-300 transition-colors">Median 6.42</span>
<span className="text-sm font-mono font-bold text-white tracking-tight group-hover:text-teal-200 transition-colors"><AnimatedNumber value={stats?.average_sgpa || 0} decimals={2} /></span>
</div>
</div>
</div>
</div>
...
<span className="text-sm font-mono font-bold text-white tracking-tight group-hover:text-emerald-300 transition-colors"><AnimatedNumber value={stats?.total_students || 0} /></span>
...
<a className="btn-kinetic w-full sm:w-auto px-7 py-3 rounded-full bg-white text-brand-pitch font-semibold text-xs tracking-wide shadow-[0_0_30px_rgba(255,255,255,0.4)] hover:bg-zinc-100 hover:shadow-[0_0_45px_rgba(255,255,255,0.7)] flex items-center justify-center gap-2 group" href="/leaderboard">
...
<a className="btn-kinetic w-full sm:w-auto px-7 py-3 rounded-full glass-pill text-white font-semibold text-xs tracking-wide border border-white/20 hover:bg-white/[0.09] hover:border-teal-400/40 hover:shadow-[0_0_30px_rgba(20,184,166,0.3)] flex items-center justify-center" href="/search">
            Detailed Search
          </a>
</div>
</div>
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-6"><a className="glass-card interactive-feature-card rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between group cursor-pointer border border-white/10 hover:border-emerald-400/50 block" href="/leaderboard">
...
<div className="flex items-center justify-between mb-6"><div className="feature-icon-badge w-12 h-12 rounded-2xl glass-pill border border-emerald-400/40 flex items-center justify-center text-emerald-300 shadow-[0_0_20px_rgba(16,185,129,0.3)] bg-emerald-500/10 transition-all duration-400"><svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.504-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.004-4.125a4.5 4.5 0 00-7.008 0M12 3v3.75m0 0a3.75 3.75 0 013.75 3.75v.75H8.25v-.75A3.75 3.75 0 0112 6.75z" strokeLinecap="round" strokeLinejoin="round" /></svg></div><span className="feature-category-pill text-xs font-mono tracking-widest uppercase text-emerald-300 bg-emerald-950/80 px-3 py-1 rounded-full border border-emerald-400/40 backdrop-blur-sm shadow-sm transition-all duration-300">CGPA TIER</span></div>
...
<div className="explore-btn inline-flex items-center gap-2 px-4 py-2 rounded-full bg-teal-400 text-brand-pitch font-semibold text-xs tracking-wide shadow-[0_0_20px_rgba(45,212,191,0.6)] opacity-90 group-hover:opacity-100 transition-all duration-300">
<span className="">Go Explore</span>
<span className="explore-arrow transition-transform duration-300">→</span>
</div>
</div>
</a>
<a className="glass-card interactive-feature-card rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between group cursor-pointer border border-white/10 hover:border-teal-400/50 block" href="/subject">
...
<a className="glass-card interactive-feature-card rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between group cursor-pointer border border-white/10 hover:border-cyan-400/50 block" href="/search">
...
<div className="explore-btn inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-400 text-brand-pitch font-semibold text-xs tracking-wide shadow-[0_0_20px_rgba(6,182,212,0.6)] opacity-90 group-hover:opacity-100 transition-all duration-300">
<span className="">Go Explore</span>
<span className="explore-arrow transition-transform duration-300">→</span>
</div>
</div>
...
<path d="M6.49 4.22a1.05 1.05 0 0 1 1.48-.48l4.47 2.19a1.05 1.05 0 0 1 .57.94v3.13L8.79 5.78a1.05 1.05 0 0 1-2.3 0L6.49 4.22zm11.02 0a1.05 1.05 0 0 0-1.48-.48l-4.47 2.19a1.05 1.05 0 0 0-.57.94v3.13l4.22-4.22a1.05 1.05 0 0 0 2.3 0l-.0-1.56zM4.22 6.49a1.05 1.05 0 0 0-.48 1.48l2.19 4.47c.2.4.6.66 1.05.66h3.02L5.78 8.88a1.05 1.05 0 0 0 0-2.3L4.22 6.49zm15.56 0a1.05 1.05 0 0 1 .48 1.48l-2.19 4.47a1.05 1.05 0 0 1-1.05.66h-3.02l4.22-4.22a1.05 1.05 0 0 1 1.56-.09zM12 8.78l3.22 3.22H8.78L12 8.78zm0 6.44l-3.22-3.22h6.44L12 15.22zM4.22 17.51a1.05 1.05 0 0 1-.48-1.48l2.19-4.47a1.05 1.05 0 0 1 1.05-.66h3.02l-4.22 4.22a1.05 1.05 0 0 1 0 2.3l-1.56.09zm15.56 0a1.05 1.05 0 0 0 .48-1.48l-2.19-4.47a1.05 1.05 0 0 0-1.05-.66h-3.02l4.22 4.22a1.05 1.05 0 0 0 0 2.3l1.56.09zM6.49 19.78a1.05 1.05 0 0 0 1.48.48l4.47-2.19a1.05 1.05 0 0 0 .57-.94v-3.13l-4.22 4.22a1.05 1.05 0 0 0-2.3 0l-.0 1.56zm11.02 0a1.05 1.05 0 0 1-1.48.48l-4.47-2.19a1.05 1.05 0 0 1-.57-.94v-3.13l4.22 4.22a1.05 1.05 0 0 1 2.3 0l.0 1.56z" />
</svg>
...
<path d="M21.362 9.354H12V.396a.396.396 0 0 0-.716-.233L2.21 12.604a.396.396 0 0 0 .316.638H12v8.958a.396.396 0 0 0 .716.233l9.074-12.441a.396.396 0 0 0-.428-.638z" />
</svg>
...
<path d="M12 3l9 18H3l9-18z" strokeLinecap="round" strokeLinejoin="round" />

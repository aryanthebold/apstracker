  ...
  return (
    <>
      <style>{`
        ...
        .test-page-override .scroll-progress-bar {
          display: none !important;
        }

        /* Branch Standings panel overrides */
        .test-page-override .glass-panel {
          background: rgba(4, 20, 20, 0.4) !important;
          border-color: rgba(45, 212, 191, 0.1) !important;
          backdrop-filter: blur(12px) !important;
          box-shadow: 0 0 20px rgba(45, 212, 191, 0.05) !important;
        }
        .test-page-override .glass-panel:hover {
          border-color: rgba(45, 212, 191, 0.3) !important;
          box-shadow: 0 0 30px rgba(45, 212, 191, 0.15) !important;
        }
        .test-page-override .bg-accent-primary\\/5 {
          background-color: rgba(45, 212, 191, 0.05) !important;
        }
        .test-page-override .from-accent-primary\\/20 {
          --tw-gradient-from: rgba(45, 212, 191, 0.2) !important;
        }
        .test-page-override .text-accent-primary {
          color: rgb(45, 212, 191) !important;
        }
        .test-page-override .bg-accent-primary\\/10 {
          background-color: rgba(45, 212, 191, 0.1) !important;
        }

        /* Cyber Glitch Animation Styles */
        @keyframes cyber-glitch-1 {
          ...
          70% { clip-path: inset(45% 0 25% 0); transform: translate(1px, -1px); }
          80% { clip-path: inset(5% 0 90% 0); transform: translate(-2px, 2px); }
          90% { clip-path: inset(90% 0 5% 0); transform: translate(2px, -2px); }
          100% { clip-path: inset(20% 0 80% 0); transform: translate(0); }
        }
        @keyframes cyber-glitch-2 {
          ...
          60% { clip-path: inset(85% 0 10% 0); transform: translate(1px, 1px); }
          70% { clip-path: inset(40% 0 40% 0); transform: translate(-1px, -1px); }
          80% { clip-path: inset(15% 0 75% 0); transform: translate(2px, 2px); }
          90% { clip-path: inset(65% 0 15% 0); transform: translate(-2px, -2px); }
        ...
        @keyframes text-flicker {
          ...
          4% { opacity: 0.9; }
          6% { opacity: 0.4; }
          8% { opacity: 1; text-shadow: 0 0 10px #2dd4bf; }
          10% { opacity: 0.8; }
          12% { opacity: 1; }
          100% { opacity: 1; }
        }
        ...
        .glitch-effect.is-glitching {
          color: #fff !important;
          text-shadow: -2px 0 #06b6d4, 2px 0 #10b981;
          animation: text-flicker 0.4s linear both;
          transform: scale(1.04) skewX(-4deg);
        }

        .glitch-effect.is-glitching::before,
        .glitch-effect.is-glitching::after {
        ...
        .glitch-effect.is-glitching::before {
          left: 3px;
          text-shadow: -2px 0 #10b981, 0 0 15px rgba(16, 185, 129, 0.6);
          animation: cyber-glitch-1 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) both infinite;
          color: #2dd4bf;
          z-index: -1;
        }
        ...
        .glitch-effect.is-glitching::after {
          left: -3px;
          text-shadow: 2px 0 #06b6d4, 0 0 15px rgba(6, 182, 212, 0.6);
          animation: cyber-glitch-2 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) both infinite reverse;
          color: #34d399;
          z-index: -1;
        }
      `}</style>
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden" data-purpose="ambient-lighting">
        ...
        <div className="absolute inset-0 hero-smoke pointer-events-none z-[1]"></div>
        ...
        {/* Vertical Grid Lines Overlay - matching reference */}
        <div className="absolute inset-0 pointer-events-none z-[1] grid-lines-overlay"></div>

        {/* Decorative Curved Lines - matching reference organic curves */}
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-[1]" preserveAspectRatio="none" viewBox="0 0 1440 900" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 340 Q 200 280, 400 320 T 800 260 T 1200 300 T 1440 240" stroke="rgba(255,255,255,0.04)" strokeWidth="1" fill="none" className="decorative-curve" />
          <path d="M0 500 Q 300 460, 600 490 T 1100 440 T 1440 480" stroke="rgba(45,212,191,0.04)" strokeWidth="1" fill="none" className="decorative-curve" style={{ animationDelay: '-3s' }} />
          <path d="M0 650 Q 250 600, 500 640 T 900 580 T 1440 620" stroke="rgba(255,255,255,0.03)" strokeWidth="0.8" fill="none" className="decorative-curve" style={{ animationDelay: '-6s' }} />
        </svg>

        {/* Subtle Noise Texture */}
        <div className="absolute inset-0 pointer-events-none z-[2] noise-overlay"></div>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link className="flex items-center gap-2.5 group transition-transform duration-300 hover:scale-105" href="/test">
            ...
            <div className="flex flex-col">
              <span className={`text-white font-bold tracking-tight text-base flex items-center gap-1.5 group-hover:text-teal-200 transition-colors duration-300 glitch-effect ${isGlitching ? 'is-glitching' : ''}`} data-text="APS Tracker">
                APS Tracker
          ...
          <nav className="hidden md:flex items-center gap-0.5 bg-white/[0.03] backdrop-blur-sm px-1 py-0.5 rounded-full border border-white/[0.04]" data-purpose="main-navigation">
            <Link className="px-4 py-1.5 text-xs font-medium text-zinc-100 hover:text-white transition-colors duration-200" href="/test">Home</Link>
            <Link className="px-4 py-1.5 text-xs font-medium text-zinc-400 hover:text-white transition-colors duration-200" href="/leaderboard">Leaderboard</Link>
            <Link className="px-4 py-1.5 text-xs font-medium text-zinc-400 hover:text-white transition-colors duration-200" href="/subject">Subjects</Link>
            <Link className="px-4 py-1.5 text-xs font-medium text-zinc-400 hover:text-white transition-colors duration-200" href="/search">Search</Link>
            <div className="w-px h-3.5 bg-white/10 mx-1"></div>
            <Link className="px-3.5 py-1.5 text-xs font-medium text-teal-300 hover:text-teal-100 transition-colors duration-200 flex items-center gap-1" href="/admin">Over Power <span className="text-[10px]">↗</span></Link>
          </nav>
      ...
      <main className="relative z-10 pt-10 pb-20 overflow-hidden" id="hero">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative min-h-[680px] flex flex-col justify-between">
          ...
          <div className="absolute inset-0 z-0 overflow-hidden select-none pointer-events-none" data-purpose="hero-starfield-backdrop">
            {/* Original Stars with tuned subtle glow */}
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '14%', left: '8%', width: '2px', height: '2px', boxShadow: '0 0 6px rgba(255, 255, 255, 0.6), 0 0 10px rgba(45, 212, 191, 0.3)', animationDuration: '3.8s', animationDelay: '0s' }}></div>
            <div className="absolute rounded-full bg-cyan-200 star-twinkle" style={{ top: '22%', left: '24%', width: '1.5px', height: '1.5px', boxShadow: '0 0 5px rgba(6, 182, 212, 0.5)', animationDuration: '4.4s', animationDelay: '0.8s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '12%', left: '34%', width: '1.8px', height: '1.8px', boxShadow: '0 0 5px rgba(255, 255, 255, 0.5)', animationDuration: '3.4s', animationDelay: '1.4s' }}></div>
            <div className="absolute rounded-full bg-teal-200 star-twinkle" style={{ top: '10%', left: '68%', width: '2.5px', height: '2.5px', boxShadow: '0 0 8px rgba(255, 255, 255, 0.7), 0 0 14px rgba(20, 184, 166, 0.4)', animationDuration: '4.8s', animationDelay: '0.3s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '18%', left: '84%', width: '1.5px', height: '1.5px', boxShadow: '0 0 4px rgba(255, 255, 255, 0.5)', animationDuration: '5.2s', animationDelay: '1.9s' }}></div>
            <div className="absolute rounded-full bg-cyan-100 star-twinkle" style={{ top: '28%', left: '93%', width: '2.2px', height: '2.2px', boxShadow: '0 0 6px rgba(255, 255, 255, 0.6), 0 0 10px rgba(6, 182, 212, 0.35)', animationDuration: '4.1s', animationDelay: '0.5s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '48%', left: '6%', width: '2.5px', height: '2.5px', boxShadow: '0 0 7px rgba(255, 255, 255, 0.65), 0 0 12px rgba(45, 212, 191, 0.35)', animationDuration: '4.6s', animationDelay: '1.2s' }}></div>
            <div className="absolute rounded-full bg-emerald-200 star-twinkle" style={{ top: '62%', left: '16%', width: '2px', height: '2px', boxShadow: '0 0 6px rgba(52, 211, 153, 0.5)', animationDuration: '3.9s', animationDelay: '2.1s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '76%', left: '10%', width: '1.5px', height: '1.5px', boxShadow: '0 0 4px rgba(255, 255, 255, 0.45)', animationDuration: '5.5s', animationDelay: '0.7s' }}></div>
            <div className="absolute rounded-full bg-teal-100 star-twinkle" style={{ top: '72%', left: '28%', width: '2px', height: '2px', boxShadow: '0 0 6px rgba(255, 255, 255, 0.6), 0 0 10px rgba(20, 184, 166, 0.35)', animationDuration: '4.3s', animationDelay: '1.6s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '44%', left: '91%', width: '2px', height: '2px', boxShadow: '0 0 6px rgba(255, 255, 255, 0.6), 0 0 10px rgba(45, 212, 191, 0.35)', animationDuration: '4.7s', animationDelay: '2.4s' }}></div>
            <div className="absolute rounded-full bg-cyan-200 star-twinkle" style={{ top: '60%', left: '81%', width: '2.8px', height: '2.8px', boxShadow: '0 0 8px rgba(255, 255, 255, 0.7), 0 0 14px rgba(6, 182, 212, 0.4)', animationDuration: '5.1s', animationDelay: '1s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '74%', left: '72%', width: '1.5px', height: '1.5px', boxShadow: '0 0 4px rgba(255, 255, 255, 0.5)', animationDuration: '3.6s', animationDelay: '2.7s' }}></div>
            <div className="absolute rounded-full bg-emerald-100 star-twinkle" style={{ top: '70%', left: '89%', width: '2px', height: '2px', boxShadow: '0 0 6px rgba(52, 211, 153, 0.5)', animationDuration: '4.5s', animationDelay: '0.2s' }}></div>

            {/* Additional Subtle & Eye-Catching Micro Stars */}
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '6%', left: '48%', width: '1.8px', height: '1.8px', boxShadow: '0 0 5px rgba(255, 255, 255, 0.5)', animationDuration: '3.2s', animationDelay: '1.1s' }}></div>
            <div className="absolute rounded-full bg-teal-200 star-twinkle" style={{ top: '16%', left: '52%', width: '2px', height: '2px', boxShadow: '0 0 6px rgba(45, 212, 191, 0.4)', animationDuration: '4.9s', animationDelay: '2.3s' }}></div>
            <div className="absolute rounded-full bg-cyan-100 star-twinkle" style={{ top: '34%', left: '42%', width: '1.6px', height: '1.6px', boxShadow: '0 0 4px rgba(6, 182, 212, 0.4)', animationDuration: '3.7s', animationDelay: '0.4s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '38%', left: '62%', width: '1.5px', height: '1.5px', boxShadow: '0 0 4px rgba(255, 255, 255, 0.4)', animationDuration: '5.4s', animationDelay: '1.8s' }}></div>
            <div className="absolute rounded-full bg-emerald-200 star-twinkle" style={{ top: '52%', left: '48%', width: '2.2px', height: '2.2px', boxShadow: '0 0 7px rgba(52, 211, 153, 0.45)', animationDuration: '4.2s', animationDelay: '3.1s' }}></div>
            <div className="absolute rounded-full bg-teal-100 star-twinkle" style={{ top: '68%', left: '44%', width: '1.8px', height: '1.8px', boxShadow: '0 0 5px rgba(20, 184, 166, 0.4)', animationDuration: '3.5s', animationDelay: '1.5s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '82%', left: '54%', width: '1.5px', height: '1.5px', boxShadow: '0 0 4px rgba(255, 255, 255, 0.45)', animationDuration: '4.7s', animationDelay: '0.9s' }}></div>
            
            {/* Edge Fill Stars */}
            <div className="absolute rounded-full bg-cyan-200 star-twinkle" style={{ top: '8%', left: '18%', width: '1.6px', height: '1.6px', boxShadow: '0 0 5px rgba(6, 182, 212, 0.4)', animationDuration: '4.0s', animationDelay: '1.7s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '32%', left: '14%', width: '1.5px', height: '1.5px', boxShadow: '0 0 4px rgba(255, 255, 255, 0.4)', animationDuration: '5.0s', animationDelay: '2.8s' }}></div>
            <div className="absolute rounded-full bg-teal-200 star-twinkle" style={{ top: '85%', left: '20%', width: '2px', height: '2px', boxShadow: '0 0 6px rgba(45, 212, 191, 0.4)', animationDuration: '3.8s', animationDelay: '0.6s' }}></div>
            <div className="absolute rounded-full bg-white star-twinkle" style={{ top: '8%', left: '82%', width: '1.8px', height: '1.8px', boxShadow: '0 0 5px rgba(255, 255, 255, 0.5)', animationDuration: '4.6s', animationDelay: '2.0s' }}></div>
            <div className="absolute rounded-full bg-emerald-100 star-twinkle" style={{ top: '36%', left: '78%', width: '1.5px', height: '1.5px', boxShadow: '0 0 4px rgba(52, 211, 153, 0.4)', animationDuration: '3.3s', animationDelay: '1.3s' }}></div>
            <div className="absolute rounded-full bg-cyan-100 star-twinkle" style={{ top: '82%', left: '84%', width: '2.1px', height: '2.1px', boxShadow: '0 0 6px rgba(6, 182, 212, 0.4)', animationDuration: '4.4s', animationDelay: '2.5s' }}></div>
          </div>
          ...
          <div className="relative z-20 flex flex-col items-center text-center max-w-3xl mx-auto mt-6 sm:mt-12">
            ...
            <div className="relative mb-6">
              ...
              <h1
                className={`text-5xl sm:text-7xl md:text-8xl lg:text-9xl tracking-tight leading-none select-none font-semibold cursor-default transition-all duration-500 glitch-effect ${isGlitching ? 'is-glitching' : 'text-white flicker-neon-title hover:brightness-110'}`}
                style={{ letterSpacing: '-0.035em' }}
                data-text="APS Tracker"
              >
                APS Tracker
              </h1>
            </div>
      ...
      <section className="relative z-20 py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto" id="notices">
        <div className="glass-pill rounded-3xl p-8 border border-white/10 relative overflow-hidden group hover:border-emerald-400/30 transition-all duration-500">
          <div className="absolute top-0 left-0 w-40 h-40 bg-emerald-500/10 group-hover:bg-emerald-500/20 rounded-full blur-3xl -ml-10 -mt-10 pointer-events-none transition-all duration-700"></div>
          <div className="relative z-10 flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-400/30 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.2)]">
              <svg className="w-5 h-5 text-emerald-300" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
          ...
          <div className="relative z-10 space-y-4">
            <div className="glass-pill p-5 rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.8)]"></span>
                <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider">Latest Update • Just now</span>
              </div>
              <p className="text-sm text-zinc-300 leading-relaxed">
                Welcome to the new APS Tracker frontend! All systems are operational. Leaderboard rankings are live. Enjoy the buttery smooth experience and the reimagined interface.
              </p>
            </div>
          </div>
        </div>
